...
var MyApp = san.defineComponent({
   template: '<div><div>' + '<input type="" value="{= name =}" placeholder="姓名（string）">' + '<input type="number" value="{= age =}" placeholder="年龄（number）">' + '<input type="" value="{= des =}" placeholder="简介"></div>' + '<div><p>信息：</p><button on-click="reset">移除信息</button></div>' + '<div><label>姓名：</label><p class="show">{{name}}</p></div>' + '<div><label>年龄：</label><p class="show">{{age}}</p></div>' + '<div><label>简介：</label><p class="show">{{des}}</p></div>' + '</div>',
   /*数据初始化*/
...
},{"san":7}],8:[function(require,module,exports) {
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + '26718' + '/');
...
//# sourceMappingURL=/demo.5ee2979f.map